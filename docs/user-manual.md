# VoiceStick M5StickC Plus 用户手册

适用版本：0.3.4
适用硬件：M5Stack M5StickC Plus

VoiceStick v0.3.4 是面向 M5Stack M5StickC Plus 的 Windows 测试与发布版本。设备负责采集音频和发送按键事件，Windows 桌面端负责连接设备、识别语音、输出文字，或触发微信电脑端语音输入。

StickS3 仍保留为仓库兼容目标，但本手册以 M5StickC Plus 的正式使用流程为准。
...
StickS3 固件仍保留在工程中，标准 Release workflow 可生成 OTA 与 merged 固件。但 v0.3.4 本次正式资料和下载包以 M5StickC Plus 为主；普通用户请优先使用上面的 M5StickC Plus 固件包。

## 3. M5StickC Plus 首次配对
